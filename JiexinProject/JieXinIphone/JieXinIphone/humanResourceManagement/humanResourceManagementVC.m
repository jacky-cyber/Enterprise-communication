...
#import "humanResourceManagementCell.h"
#import "humanResourceDetail.h"
#import "myButton.h"
#import "myTitleLabel.h"
@interface humanResourceManagementVC (){
    int currentPage;
    BOOL refreshData;
    int RequestTag;
    NSString *nameTitle;
...
@end
@implementation humanResourceManagementVC
@synthesize fModeArr;
...
- (void)loadlistView
{
    ...
    myTitleLabel *titleView=[[myTitleLabel alloc]initTitleLabel:nameTitle rect:CGRectMake(40, self.iosChangeFloat, 200,44) fontSize:17 fontColor:RGBCOLOR(101, 99,100)];
...
-(void)loadNoMessage:(NSArray*)arr{
    if(arr==nil||[arr count]==0){
        myTitleLabel *titleLabe=[[myTitleLabel alloc]initTitleLabel:@"暂时没有内容" rect:CGRectMake(80,230,150,30) fontSize:17 fontColor:RGBCOLOR(100, 100, 100)];
...
- (void)requestDataForServer:(int)pageNumber{
    ...
    [[HttpServiceHelper sharedService] requestForType:kQueryhumanResourcelist info:@{@"pageNumber":[NSString stringWithFormat:@"%d",pageNumber],@"pageCount":[NSString stringWithFormat:@"%d",pageCount],@"category":[NSString stringWithFormat:@"%d",RequestTag]} target:self successSel:@"requestFinished:" failedSel:@"requestFailed:"];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    humanResourceManagementCell *cell = [tableView dequeueReusableCellWithIdentifier:cellIdentifier];
    if (cell == nil) {
        cell = [[[humanResourceManagementCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:cellIdentifier] autorelease];
...
-(void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{
    humanResourceDetail *connetVC = [[humanResourceDetail alloc] initWithcourseid:[fModeArr[indexPath.row] objectForKey:@"courseid"]];
