...
@interface companyNewsDetailView ()
- (void)resquestData:(NSString*)courseid; //请求财务详情数据
@end

@implementation companyNewsDetailView
...
- (void)viewDidLoad
{
    ...
    _backLabel.text = @"新闻详情";
    _backLabel.font = [UIFont systemFontOfSize:17.0f];
    [self.view addSubview:_backLabel];
    
}
- (void)resquestData:(NSString *)courseid{
    [[HttpServiceHelper sharedService] requestForType:kQuerynewsDetail info:@{@"courseid":[NSString stringWithFormat:@"%@",courseid]} target:self successSel:@"requestDetailFinished:" failedSel:@"requestDetailFailed:"];
}//请求财务详情数据
-(void)requestDetailFinished:(NSDictionary *)datas{
    ...
    NSString*str = [NSString stringWithFormat:@"http://%@:8087/newszq/phoneInterface.action",sa];
