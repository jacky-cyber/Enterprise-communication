...
@interface SecurityTipsViewController ()
@end

@implementation SecurityTipsViewController
@synthesize fModeArr;
...
- (void)requestDataForServer:(int)pageNumber{
    int pageCount=10;
    [[HttpServiceHelper sharedService] requestForType:kQuerySecurityTips info:@{@"pageNumber":[NSString stringWithFormat:@"%d",pageNumber],@"pageCount":[NSString stringWithFormat:@"%d",pageCount]} target:self successSel:@"requestFinished:" failedSel:@"requestFailed:"];
...
- (void)requestFinished:(NSDictionary *)datas
{
    ...
    for (int i=0; i<[fModeArr count]; i++) {
        NSString * picStr = [NSString stringWithFormat:@"http://%@:8087/saferzq/page/titleImg/%@",str,[[fModeArr objectAtIndex:i] objectForKey:@"summary"]];
...
-(UIImage*)loagImageIndex:(int)index{
    ...
    NSString * picStr = [NSString stringWithFormat:@"http://%@:8087/saferzq/page/titleImg/%@",str,[[fModeArr objectAtIndex:index] objectForKey:@"summary"]];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    //创建单元格
    if (cell == nil) {
        cell = [[[SecurityCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:cellIdentifier] autorelease];
    ...
    NSString * picStr = [NSString stringWithFormat:@"http://%@:8087/saferzq/page/titleImg/%@",str,[[fModeArr objectAtIndex:indexPath.row] objectForKey:@"summary"]];
    NSLog(@"pic%@",picStr);
...
-(void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{
    SecurityTipsDetailViewController *cView = [[SecurityTipsDetailViewController alloc] initWithcourseid:[fModeArr[indexPath.row] objectForKey:@"courseid"]];
...
-(CGFloat)tableView:(UITableView *)tableView heightForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    NSString * picStr = [NSString stringWithFormat:@"http://%@:8087/saferzq/page/titleImg/%@",str,[[fModeArr objectAtIndex:indexPath.row] objectForKey:@"summary"]];
    if ([picStr isEqualToString:@"http://111.11.28.30:8087/saferzq/page/titleImg/"]||[picStr isEqualToString:@"http://111.11.28.9:8087/saferzq/page/titleImg/"]) {
