...
//    for (NSTextCheckingResult* b in chunks)
//    {
//        NSString *partStr = [markup substringWithRange:b.range];
//        NSString *originalStr = [self escapedString:partStr];
//
//        CTFontRef fontRef = CTFontCreateWithName((CFStringRef)self.font,
//                                                 24.0f, NULL);
//        //apply the current text style //2
...
//        
//        
//        if (![partStr hasPrefix:@"<font"]&&![partStr hasPrefix:@"<img"]) {
//            NSAttributedString *partString = [[NSAttributedString alloc] initWithString:originalStr attributes:attrs];
//            [string appendAttributedString:partString];
//            [partString release];
//            CFRelease(fontRef);
//        }
//        else if([partStr hasPrefix:@"<font"])
//        {
//             NSRegularExpression* scolorRegex = [[[NSRegularExpression alloc] initWithPattern:@"(?<=strokeColor=[\'\"])\\w+" options:0 error:NULL] autorelease];
//            [scolorRegex enumerateMatchesInString:originalStr options:0 range:NSMakeRange(0, [originalStr length]) usingBlock:^(NSTextCheckingResult *match, NSMatchingFlags flags, BOOL *stop){
//                if ([[originalStr substringWithRange:match.range] isEqualToString:@"none"]) {
//                    self.strokeWidth = 0.0;
//                } else {
//                    self.strokeWidth = -2.0;
//                    SEL colorSel = NSSelectorFromString([NSString stringWithFormat: @"%@Color", [originalStr substringWithRange:match.range]]);
//                    self.strokeColor = [UIColor performSelector:colorSel];
//                }
//            }];
//            //color
//            NSRegularExpression* colorRegex = [[[NSRegularExpression alloc] initWithPattern:@"(?<=color=[\'\"])\\w+" options:0 error:NULL] autorelease];
//            [colorRegex enumerateMatchesInString:originalStr options:0 range:NSMakeRange(0, [originalStr length]) usingBlock:^(NSTextCheckingResult *match, NSMatchingFlags flags, BOOL *stop){
//                SEL colorSel = NSSelectorFromString([NSString stringWithFormat: @"%@Color", [originalStr substringWithRange:match.range]]);
//                self.color = [UIColor performSelector:colorSel];
//            }];
//            
//            //face
//            NSRegularExpression* faceRegex = [[[NSRegularExpression alloc] initWithPattern:@"(?<=face=[\'\"])[^[\'\"]]+" options:0 error:NULL] autorelease];
//            [faceRegex enumerateMatchesInString:originalStr options:0 range:NSMakeRange(0, [originalStr length]) usingBlock:^(NSTextCheckingResult *match, NSMatchingFlags flags, BOOL *stop){
//                self.font = [originalStr substringWithRange:match.range];
//            }];
//
//        }
//        else if ([partStr hasPrefix:@"<img"])
//        {
//            __block NSNumber* width = [NSNumber numberWithInt:0];
//            __block NSNumber* height = [NSNumber numberWithInt:0];
//            __block NSString* fileName = @"";
...
//            //width
//            NSRegularExpression* widthRegex = [[[NSRegularExpression alloc] initWithPattern:@"(?<=width=[\'\"])[^[\'\"]]+" options:0 error:NULL] autorelease];
//            [widthRegex enumerateMatchesInString:originalStr options:0 range:NSMakeRange(0, [originalStr length]) usingBlock:^(NSTextCheckingResult *match, NSMatchingFlags flags, BOOL *stop){
//                width = [NSNumber numberWithInt: [[originalStr substringWithRange: match.range] intValue] ];
//            }];
//
//            //height
//            NSRegularExpression* faceRegex = [[[NSRegularExpression alloc] initWithPattern:@"(?<=height=[\'\"])[^[\'\"]]+" options:0 error:NULL] autorelease];
//            [faceRegex enumerateMatchesInString:originalStr options:0 range:NSMakeRange(0, [originalStr length]) usingBlock:^(NSTextCheckingResult *match, NSMatchingFlags flags, BOOL *stop){
//                height = [NSNumber numberWithInt: [[originalStr substringWithRange:match.range] intValue]];
//            }];
//            
//            //image
//            NSRegularExpression* srcRegex = [[[NSRegularExpression alloc] initWithPattern:@"(?<=src=[\'\"])[^[\'\"]]+" options:0 error:NULL] autorelease];
//            [srcRegex enumerateMatchesInString:originalStr options:0 range:NSMakeRange(0, [originalStr length]) usingBlock:^(NSTextCheckingResult *match, NSMatchingFlags flags, BOOL *stop){
//                fileName = [originalStr substringWithRange: match.range];
//            }];
//            
//            //add the image for drawing
//            [self.images addObject:
