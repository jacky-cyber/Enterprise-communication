...
- (void)parser:(NSXMLParser *)parser didEndElement:(NSString *)elementName namespaceURI:(NSString *)namespaceURI qualifiedName:(NSString *)qName
{
    ...
    else if (!_foundCharacters && _elementStarted)
    {
        NSString *objectPath = [_currentDepth componentsJoinedByString:@"."];
        [_resultObject removeObjectForKey:objectPath];
        [_resultObject setValue:nil forKey:objectPath];
    }
...
//    NSLog(@"%@",[_currentDepth componentsJoinedByString:@"."]);
    ...
    if([currentDictObject objectForKey:elementName] == nil)    {
        if (!currentDict) {
            [currentDictObject setValue:nil  forKey:elementName];
