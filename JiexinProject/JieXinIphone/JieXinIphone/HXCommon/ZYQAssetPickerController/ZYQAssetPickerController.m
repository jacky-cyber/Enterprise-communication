...
@interface ZYQAssetViewController ()<ZYQAssetViewCellDelegate,UITableViewDataSource,UITableViewDelegate,PhotoLibraryDelegate>{
    int columns;
...
- (id)init
{
    ...
    self.listTableView = [[UITableView alloc] initWithFrame:CGRectMake(0, 0, kScreen_Width, kScreen_Height-44-20) style:UITableViewStylePlain];
    ...
    {
        _listTableView.contentInset=UIEdgeInsetsMake(9.0, 2.0, 0, 2.0);
    ...
    {
        _listTableView.contentInset=UIEdgeInsetsMake(9.0, 0, kBottomHeight, 0);
...
- (void)initBottomView
{
    CGFloat bottomTop = kScreen_Height-20-44-kBottomHeight;
    if (IOSVersion>=7.0) {
        bottomTop = kScreen_Height-kBottomHeight;
    }
    self.bottomView= [[UIImageView alloc] initWithFrame:CGRectMake(0, bottomTop, 320, kBottomHeight)];
    _bottomView.userInteractionEnabled = YES;
    _bottomView.image = [UIImage imageNamed: @"bottomBg.png"];
    [self.view addSubview:_bottomView];
    
    
    self.rotateBtn = [UIButton buttonWithType:UIButtonTypeCustom];
    _rotateBtn.frame = CGRectMake(0, 0, 100, kBottomHeight);
    [_rotateBtn setImage:[UIImage imageNamed:@"rotateBtn1.png"] forState:UIControlStateNormal];
    _rotateBtn.enabled = NO;
    [_rotateBtn setImage:[UIImage imageNamed:@"rotateBtn2.png"] forState:UIControlStateSelected];
    [_rotateBtn setImageEdgeInsets:UIEdgeInsetsMake(10, 10, 10, 30)];
    [_rotateBtn addTarget:self action:@selector(rotateBtnTap) forControlEvents:UIControlEventTouchUpInside];
    [_bottomView addSubview:_rotateBtn];
    
    self.sendBtn = [UIButton buttonWithType:UIButtonTypeCustom];
    _sendBtn.frame = CGRectMake(kScreen_Width-100, 0, 100, kBottomHeight);
    [_sendBtn setImage:[UIImage imageNamed:@"sendBtn1.png"] forState:UIControlStateNormal];
    [_sendBtn setImage:[UIImage imageNamed:@"sendBtn2.png"] forState:UIControlStateSelected];
    _sendBtn.enabled = NO;
    [_sendBtn setImageEdgeInsets:UIEdgeInsetsMake(10, 30, 10, 10)];
    [_sendBtn addTarget:self action:@selector(finishPickingAssets:) forControlEvents:UIControlEventTouchUpInside];
    [_bottomView addSubview:_sendBtn];
    
    [self setBtnState:_indexPathsForSelectedItems];
}
...
- (void)setupAssets
{
    ...
    ALAssetsGroupEnumerationResultsBlock resultsBlock = ^(ALAsset *asset, NSUInteger index, BOOL *stop) {
        ...
        else if (self.assets.count > 0)
        {
            ...
            [_listTableView scrollToRowAtIndexPath:[NSIndexPath indexPathForRow:ceil(self.assets.count*1.0/columns)  inSection:0] atScrollPosition:UITableViewScrollPositionTop animated:YES];
        }
    };
...
-(UITableViewCell*)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    [cell bind:tempAssets selectionFilter:picker.selectionFilter minimumInteritemSpacing:minimumInteritemSpacing minimumLineSpacing:minimumLineSpacing columns:columns assetViewX:(_listTableView.frame.size.width-kThumbnailSize.width*kRowNum-minimumInteritemSpacing*(kRowNum-1))/2];
    return cell;
}
...
- (void)setBtnState:(NSMutableArray *)arr
{
    if ([arr count] == 1) {
        _rotateBtn.enabled = YES;
        _rotateBtn.selected = YES;
    }
    else
    {
        _rotateBtn.enabled = NO;
        _rotateBtn.selected = NO;
    }
...
- (void)selectImage:(UIImage *)image withInfo:(id)info
{
    ...
    ZYQAssetPickerController *picker = (ZYQAssetPickerController *)self.navigationController;
    if ([picker.delegate respondsToSelector:@selector(assetPickerController:didFinishPickingAssets:)])
        [picker.delegate assetPickerController:picker didFinishRotateImage:image];
