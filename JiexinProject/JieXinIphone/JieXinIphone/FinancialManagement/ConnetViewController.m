...
- (void)viewDidLoad
{
    ...
    JTitleLabel *titleView=[[JTitleLabel alloc]initJTitleLabel:@"财务详情" rect:CGRectMake(40, self.iosChangeFloat, 200,44) fontSize:17 fontColor:RGBCOLOR(101, 99,100)];
...
- (void)resquestData:(NSString *)courseid{
    ...
    [[HttpServiceHelper sharedService] requestForType:kQuerycourseFinancialDetial info:@{@"courseid":[NSString stringWithFormat:@"%@",courseid] } target:self successSel:@"requestDetailFinished:" failedSel:@"requestDetailFailed:"];
}//请求财务详情数据
-(void)requestDetailFinished:(NSDictionary *)datas{
    ...
    {
        replaceStr=@"http://111.11.28.9:8087/financialMzq/page/img/postImg";
    }
    else
    {
        replaceStr=@"http://111.11.28.30:8087/financialMzq/page/img/postImg";
    }
